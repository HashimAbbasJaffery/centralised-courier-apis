...
const shipment_types = {
    ...
    "LOGISTICS": [
        ["OVERLAND LOGISTICS", "OVERLAND LOGISTICS"],
    ]
};
const materials = ref([]);
const division = ref("");
...
async function submit(event) {
    ...
    console.log(...items.value.map(item => ({
    ...
    const response = await axios.post(route('courier.createOrder', { courier: 'leopard' }), {
        destination_city: selectedCity.value.id,
    ...
    <div class="container-xxl container-p-y flex-grow-1">
        <form class="card-body" method="POST" @submit="submit">
            ...
            <div class="card mb-4 p-3">
                ...
                <div class="row g-6">
                    <Input @update:modelValue="commonForm.name = $event" label="Consignee Name *" type="text" name="consignee_name" placeholder="Consignee Name" />
                    <Input @update:modelValue="commonForm.phone = $event" label="Consignee Phone *" type="text" name="consignee_phone" placeholder="Consignee Phone" />
                    <Input @update:modelValue="commonForm.address = $event" label="Consignee Address *" type="text" name="consignee_address" placeholder="Consignee Address" :length="12" />
                </div>
            ...
            <div class="card mb-4 p-3">
                ...
                <table class="table-bordered table" id="itemsTable">
                    ...
                    <tbody>
                        <tr class="item-row" v-for="item in items" :key="item.id">
                            ...
                            <td><input type="number" step="0.01" v-model="item.purchase_cost" name="purchase_cost[]" class="form-control purchase-cost" required /></td>
                            <td><input type="number" step="0.01" v-model="item.item_price" name="item_price[]" class="form-control item-price" required /></td>
                            <td>
                            ...
                            <td><input type="number" step="0.01" name="profit[]" :value="parseInt((item.item_price - item.purchase_cost)) - getMaterialPrice(item.material_id)" class="form-control profit" readonly /></td>
                ...
                <div class="pt-6" style="display: flex; justify-content: flex-start;">
                    <button :disabled="is_loading" type="submit" class="btn btn-primary waves-effect waves-light me-4 d-flex align-items-center justify-content-center" style="min-width: 120px;">
